...
class Scribd {
    ...
    public function __construct($api_key, $secret) {
        ...
        $this->secret = $secret;
        $this->url = "api.scribd.com/api?api_key=".$api_key;
     }
   ...
   */
   ...
   /**
   * Get Url from  a doc_id  uploaded to the webservice
   * @return string providing a url with the flash object to view the file ;
   */
      public function getEmbedUrlFlash($doc_id,$access_key){
     if(!empty($doc_id) && !empty($access_key)){
            $EmbedUrl = "http://d1.scribdassets.com/ScribdViewer.swf?document_id={$doc_id}&access_key={$access_key}&page=1&version=1&viewMode=list";
        }else{
                $EmbedUrl="Url for embeding file could not be generated";              
            }
         return $EmbedUrl;
    }
       
     /**
     * Get all doc_id list that currently are uploaded on the webservice
     * @return array containing all doc_id;
     */ 
       public function getListDocId() {
            $this->url='http://api.scribd.com/api?method=docs.getList&api_key= ' . $this->api_key;
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL,  $this->url);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER,TRUE);
            $result=curl_exec($ch); //we return a string
            curl_close($ch);
            $xmlObject=simplexml_load_string($result);
            foreach ( (array) $xmlObject as $index => $node )
            $out[$index] = ( is_object ( $node ) ) ? xml2array ( $node ) : $node;
            $stack = array();
            foreach($out["resultset"]["result"] as $index => $oItem){
                array_push($stack,$oItem->doc_id);
            }
            return $stack;    
        }
